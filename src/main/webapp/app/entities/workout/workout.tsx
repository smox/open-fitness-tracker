  ...
  return (
    <div>
      <h2 id="workout-heading">
        <Translate contentKey="openfitnesstrackerApp.workout.home.title">Workouts</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="openfitnesstrackerApp.workout.home.createLabel">Create new Workout</Translate>
        </Link>
      </h2>
        ...
        {workoutList && workoutList.length > 0 ? (
            ...
            <tbody>
              {workoutList.map((workout, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{workout.language ? <Link to={`language/${workout.language.id}`}>{workout.language.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="openfitnesstrackerApp.workout.home.notFound">No Workouts found</Translate>
            </div>
          )
        )}
      </div>
